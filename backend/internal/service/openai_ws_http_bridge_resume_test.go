...
func TestOpenAIWSHTTPBridgeLaterTurn429RetriesCurrentTurnOnReplacementAccount(t *testing.T) {
	...
	require.Equal(t, firstFingerprint.sessionID, gjson.GetBytes(upstream.bodies[1], "client_metadata.session_id").String())
	require.Equal(t, firstFingerprint.threadID, gjson.GetBytes(upstream.bodies[1], "client_metadata.thread_id").String())
	require.NotContains(t, string(upstream.bodies[2]), "previous_response_id")
	require.Contains(t, string(upstream.bodies[2]), "second")
	require.Equal(t, replacementFingerprint.sessionID, gjson.GetBytes(upstream.bodies[2], "client_metadata.session_id").String())
	require.Equal(t, replacementFingerprint.threadID, gjson.GetBytes(upstream.bodies[2], "client_metadata.thread_id").String())
