...
const (
	...
	// Codex TUI initializes the embedded app server with this client name. The
	// app server appends the matching name/version pair to the base User-Agent.
	codexCLIClientName = "codex-tui"
	// codexCLIUserAgent 是编译期兜底 UA；运行时优先使用由后台版本号拼出的规范 UA。
	// 版本段必须来自 codexCLIVersion，避免 UA 内两个版本位置彼此漂移。
	...
	// codexCLIVersion 是网关编译期支持的 Codex 客户端版本，供 User-Agent、
	// /models 的 client_version 以及自动同步关闭或失败时兜底使用。
