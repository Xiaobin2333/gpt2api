...
func (s *OpenAIOAuthServiceSuite) TestExchangeCode_DefaultRedirectURI() {
	...
	s.setupServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if _, ok := r.Header["User-Agent"]; ok {
			errCh <- "unexpected user-agent"
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		if _, ok := r.Header["Originator"]; ok {
			errCh <- "unexpected originator"
			w.WriteHeader(http.StatusBadRequest)
			return
		}
...
func (s *OpenAIOAuthServiceSuite) TestRefreshToken_JSONBody() {
	errCh := make(chan string, 1)
	s.setupServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		wantBody := `{"client_id":"` + openai.ClientID + `","grant_type":"refresh_token","refresh_token":"rt"}`
