-- Converge every existing OpenAI OAuth account across all supported identity carriers.
-- Preserve canonical seeds so changing scope does not rotate the upstream installation.
UPDATE accounts
SET extra = jsonb_set(
    jsonb_set(
        CASE
