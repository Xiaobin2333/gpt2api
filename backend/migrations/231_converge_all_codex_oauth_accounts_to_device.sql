-- Bind every existing OpenAI OAuth account to a stable installation identity only.
-- Preserve canonical seeds so changing scope does not rotate the upstream device.
UPDATE accounts
SET extra = jsonb_set(
    jsonb_set(
        CASE
