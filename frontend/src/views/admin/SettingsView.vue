      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          ...
          <div class="space-y-5 p-6">
            ...
            <div class="border-t border-gray-100 pt-4 dark:border-dark-700">
              <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                {{ t('admin.settings.features.riskControl.cyberSessionBlockGroups') }}
              </label>
              <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.settings.features.riskControl.cyberSessionBlockGroupsHint') }}
              </p>
              <div v-if="cyberSessionBlockGroups.length === 0" class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.settings.features.riskControl.cyberSessionBlockNoGroups') }}
              </div>
              <div v-else class="mt-3 space-y-2">
                <div
                  v-for="group in cyberSessionBlockGroups"
                  :key="group.id"
                  class="flex items-center justify-between rounded border border-gray-200 px-3 py-2 dark:border-dark-600"
                >
                  <span class="text-sm text-gray-700 dark:text-gray-200">{{ group.name }}</span>
                  <Toggle
                    :model-value="isCyberSessionBlockGroupEnabled(group.id)"
                    @update:model-value="setCyberSessionBlockGroupPolicy(group.id, $event)"
                  />
                </div>
              </div>
            </div>
...
const form = reactive<SettingsForm>({
  ...
  cyber_session_block_group_policies: [] as CyberSessionBlockGroupPolicy[],
...
async function loadSubscriptionGroups() {
  try {
    ...
    cyberSessionBlockGroups.value = groups.filter((group) => group.status === "active");
...
function isCyberSessionBlockGroupEnabled(groupID: number): boolean {
  const policy = form.cyber_session_block_group_policies.find(
    (item) => item.group_id === groupID,
  );
  return policy ? policy.enabled : form.cyber_session_block_enabled;
}

function setCyberSessionBlockGroupPolicy(groupID: number, enabled: boolean): void {
  const index = form.cyber_session_block_group_policies.findIndex(
    (policy) => policy.group_id === groupID,
  );
  if (index >= 0) {
    form.cyber_session_block_group_policies[index].enabled = enabled;
    return;
  }
  form.cyber_session_block_group_policies.push({ group_id: groupID, enabled });
}
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      cyber_session_block_group_policies: form.cyber_session_block_group_policies
        .filter((policy) => Number.isInteger(policy.group_id) && policy.group_id > 0)
        .map((policy) => ({ group_id: policy.group_id, enabled: Boolean(policy.enabled) })),
